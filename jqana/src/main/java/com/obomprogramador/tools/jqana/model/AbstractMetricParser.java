...
/**
 * This is the abstract parser used as a base class for all parsers.
...
public abstract class AbstractMetricParser implements Parser {
    ...
    /**
     * Default constructor.
     * @param packageMeasurement Measurement the measurement to be used.
     * @param context Context the running context.
     * @param metricResourceId String current metric's resource Id.
     */
    public AbstractMetricParser(Measurement packageMeasurement,
            Context context, String metricResourceId) {
        logger = LoggerFactory.getLogger(this.getClass());
        this.context = context;
        this.packageMeasurement = packageMeasurement;
        this.metricResourceId = metricResourceId;
        this.metric = context.getCurrentMetric(context.getBundle().getString(
                metricResourceId));
        if (this.metric == null) {
            throw new IllegalArgumentException(
                    "Context is not valid. Metric is null.");
        }
    }
    ...
    public Measurement parse(String compiledClassPath, String sourceCode) {
        this.measurement = new Measurement(); // Class name will be set inside
                                              // listener.
        ...
        try {
            ...
            logger.debug("**** " + this.metric.getMetricName() + ": "
     ...
     * 
     * @param p JavaParser the parser being used.
     * @return JavaBaseListener this is only a hook for overrides.
     */
    ...
    /**
     * This is a template method hook. What do you want to do after parsing the
     * source file?
     */
    ...
    protected void updatePackageMeasurement() {
        ...
        // 1 - Add ths chass' measurements to the package's measurements
        // collection:

        int indx = this.packageMeasurement.getInnerMeasurements().indexOf(
                this.measurement);
        if (indx >= 0) {
            // Collection of inner measurements already has a measurement of
            // this class. Ok.
            classMeasurement = this.packageMeasurement.getInnerMeasurements()
                    .get(indx);
        } else {
            // It is the first measurement of this class:
            classMeasurement = this.measurement;
            this.packageMeasurement.getInnerMeasurements()
                    .add(this.measurement);
        }

        // 2 - Add this class' metric value to package's metric values,
        ...
        if (this.packageMeasurement.getMetricValues().contains(mv)) {
            // This package already contains a CC metric value, so, lets add to
            // it:
    ...
    /**
     * Part of the template method "updatePackageMeasurement()" that aggregates
     * the metric value.
     * 
     * @param packageMv MetricValue - the package's metrics. 
     * @param mv MetricValue - Current metrics. 
     */
