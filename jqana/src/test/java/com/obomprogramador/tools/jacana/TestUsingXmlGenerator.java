...
public class TestUsingXmlGenerator {
    ...
    public void test() throws ClassNotFoundException, InstantiationException,
        ...
        try {
            ...
            File sourceDir = new File(this.getClass().getClassLoader()
                    .getResource(rootTestResources).toURI());
            ...
            fail(">>>>>>>>>> jQana Report Error: "
                    + context.getStatusBeforeException() + ". Exception: "
                    + ex.getMessage());
        }
    ...
    private boolean verifyResult(Document report) throws JAXBException {
        ...
        for (int x = 0; x < testClasses.length; x++) {
            ...
            Measurement classM = getMeasurement(
                    packageM.getInnerMeasurements(), testClasses[x][1]);
            if (classM == null) {
            ...
            MetricValue ccMetric = classM.getMetricValue(context.getBundle()
                    .getString("metric.cc.name"));
            if (ccMetric == null) {
                System.out.println("*** ERROR ccMetric is missing. Class: "
                        + testClasses[x][1]);
                result = false;
                break;
            }
            MetricValue lcom4Metric = classM.getMetricValue(context.getBundle()
                    .getString("metric.lcom4.name"));
            if (ccMetric == null) {
                System.out.println("*** ERROR lcom4Metric is missing. Class: "
                        + testClasses[x][1]);
                result = false;
                break;
            }
            MetricValue rfcMetric = classM.getMetricValue(context.getBundle()
                    .getString("metric.rfc.name"));
            if (ccMetric == null) {
                System.out.println("*** ERROR rfcMetric is missing. Class: "
                        + testClasses[x][1]);
                result = false;
                break;
            }
            ...
            try {
                ...
                if (Math.abs(ccValue - ccMetric.getValue()) > 0.5) {
                    System.out.println("*** ERROR metric CC value for class: "
                            + testClasses[x][1] + " expected: " + ccValue
                            + " found: " + ccMetric.getValue());
                    result = false;
                }
                if (Math.abs(lcom4Value - lcom4Metric.getValue()) > 0.5) {
                    System.out
                            .println("*** ERROR metric lcom4 value for class: "
                                    + testClasses[x][1] + " expected: "
                                    + lcom4Value + " found: "
                                    + lcom4Metric.getValue());
                    result = false;
                }
                if (Math.abs(rfcValue - rfcMetric.getValue()) > 0.5) {
                    System.out.println("*** ERROR metric rfc value for class: "
                            + testClasses[x][1] + " expected: " + rfcValue
                            + " found: " + rfcMetric.getValue());
                    result = false;
                }

            } catch (NumberFormatException nfe) {
                System.out.println("*** ERROR invalid metric value for class: "
                        + testClasses[x][1] + " value: " + value);
                result = false;
            }
        }
        return result;
    ...
    private Measurement getMeasurement(List<Measurement> measurements,
            String name) {
